...
interface PixelCanvasProps extends React.HTMLAttributes<HTMLDivElement> {
    ...
    noFocus?: boolean;
    /** Track pointer via window so the canvas can sit behind interactive content */
    overlay?: boolean;
    /** Variant style */
...
export function PixelCanvas({
    ...
    useEffect(() => {
        ...
        const draw = (timestamp: number) => {
            ...
            const alphaScale = overlay ? 0.55 : 0.9;
                    ...
                    if (pixel.intensity > 0.01) {
                        ...
                        if (variant === "glow" && pixel.intensity > 0.2) {
                            for (let g = glowPasses; g > 0; g--) {
                                ...
                                ctx.globalAlpha = (pixel.intensity * 0.15 / g) * (overlay ? 0.6 : 1);
                                ctx.fillStyle = color;
                                ctx.fillRect(
                                    pixel.x - glowOffset,
                                    pixel.y - glowOffset,
                        ...
                        // Main pixel
                        ctx.globalAlpha = pixel.intensity * alphaScale;
                        ctx.fillStyle = color;
        ...
        const onTouchMove = (e: TouchEvent) => {
            if (e.touches.length > 0) {
                ...
                if (touch) {
                    pointFromEvent(touch.clientX, touch.clientY);
                }
            }
        };
    ...
    }, [gap, speed, noFocus, overlay, variant, getColorFromIntensity]);
